...
export default markRaw({
  ...
  methods: {
    ...
    registerPerformanceCategory() {
      axios.post(`${process.env.API_URL}/perform/save`, { name: this.name })
        .then(() => {
    ...
    getPerformancedetail(perfid){
      axios.get(`${process.env.API_URL}/perform-detail/${perfid}`)
        .then(response=>{
   ...
   registerPerformanceDetail(){
      ...
      axios.post(`${process.env.API_URL}/perform-detail/save`,{
